...
router.get('/', async (req, res) => {
    res.sendFile(path.join(__dirname, '/../resources/userDelete.html'))
});
...
router.post('/', async (req, res) => {
    ...
    const result = await deleteUser(userId, userPw);

    // 3. DB 요청 결과를 통해 어떤 화면과 연결시킬지 판단 및 결정.
    if (result === "성공") {
        // 가입된 계정으로 로그인한 화면으로 이동시키기
        console.log("삭제가 완료되었습니다.. 자동으로 로그아웃 합니다.")
        res.redirect('/')
    } else if (result === '해당ID/PW가 없음'){
        console.log('해당ID/PW가 없음')
        res.render('userDelete',{'userId': userId, 'userPw':userPw})
    } else if (result === "실패") {
...
async function deleteUser(userId, userPw){
    let connection;
    try {
