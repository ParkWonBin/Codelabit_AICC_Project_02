 ...
 * .error - 에러여부 혹은 에러내역
 */
const db_bulletinCreateComments = async (writerId, postId, commentContent, parentId) => {
    let connection;
    ...
    try {
        ...
        const sqlString = `
            INSERT INTO comments (comment_id, post_id, writer_id, content${parentId ? ", parent_comment_id" : ""})
            VALUES (comments_seq.NEXTVAL, ${parseInt(postId)}, :writerId, :content${parentId ? `, ${parseInt(parentId)}` : ""})
        `
        const bindData = {
            writerId:writerId,
...
module.exports = db_bulletinCreateComments;