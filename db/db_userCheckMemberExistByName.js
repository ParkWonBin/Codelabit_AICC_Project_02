 ...
 * .error - 에러여부 혹은 에러내역
 */
async function db_userCheckMemberExistByName(memberName){
    let connection;
    ...
    try {
        ...
        const sqlString =  'SELECT count(*) FROM member WHERE member_name = :memberName';
        const bindData = {'memberName':memberName}
        ...
        console.error(`${db_userCheckMemberExistByName.name} 오류 발생:${error}`);
        return {
            memberExist: null,
            error:error
        };
...
module.exports = db_userCheckMemberExistByName;